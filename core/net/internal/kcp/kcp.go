...
func (kcp *KCP) newDataBuffer(size int) (data []byte) {
	data = xmitBuf.Get().([]byte)[:size]
	return
}

func (kcp *KCP) delDataBuffer(data []byte) {
	xmitBuf.Put(data)
}
...
func init() {
	...
	//	f, err := os.Create("./retran/" + now) //创建文件
	//	if nil != err {
...
func (kcp *KCP) flush(ackOnly bool) {
	...
	if fastRetransSegs > 0 {
		...
		//		writer.Write([]byte("快速重传:" + strconv.Itoa(int(DefaultSnmp.FastRetransSegs))))
	}
	if earlyRetransSegs > 0 {
		atomic.AddUint64(&DefaultSnmp.EarlyRetransSegs, earlyRetransSegs)
		sum += earlyRetransSegs

		//		writer.Write([]byte("early重传:" + strconv.Itoa(int(DefaultSnmp.FastRetransSegs))))
	}
	if sum > 0 {
		atomic.AddUint64(&DefaultSnmp.RetransSegs, sum)

		//		writer.Write([]byte("总重传:" + strconv.Itoa(int(DefaultSnmp.FastRetransSegs))))
	}
...
func (kcp *KCP) CheckNewInterval(current uint32) (interval time.Duration) {
	...
	interval = time.Duration(minimal) * time.Millisecond
	return
}
