...
func dialGame() (*UDPSession, error) {
	block, _ := NewSalsa20BlockCrypt(pass)
	sess, err := DialWithOptions(portGame, block, 10, 3)
	if err != nil {
		panic(err)
	}
...
//func listenGame() (net.Listener, error) {
//	block, _ := NewSalsa20BlockCrypt(pass)
//	return ListenWithOptions(portGame, block, 10, 3)
//}

func gameServer() {
	l, err := listenGameForTest()
	if err != nil {
		panic(err)
	}
	listener := l.(*Listener)
...
///////////////////////////
func TestGameserverSndRcv(t *testing.T) {
	cli, err := dialGame()
	if err != nil {
		panic(err)
	}
